...
public record TimeCondition(int startTick, int endTick, Optional<Keyword> keyword) {
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, TimeCondition> STREAM_CODEC = StreamCodec.of(
            (buf, condition) -> {
                Optional<Keyword> keyword = condition.keyword();
                buf.writeBoolean(keyword.isPresent());
                if (keyword.isPresent()) {
                    buf.writeUtf(keyword.get().getSerializedName());
                } else {
                    buf.writeVarInt(condition.startTick());
                    buf.writeVarInt(condition.endTick());
                }
            },
            buf -> {
                boolean hasKeyword = buf.readBoolean();
                if (hasKeyword) {
                    String name = buf.readUtf(16);
                    Keyword keyword = Keyword.byName(name)
                            .orElseThrow(() -> new IllegalArgumentException("Unknown time keyword '" + name + "'."));
                    return TimeCondition.keyword(keyword);
                }
    ...
    public static final Codec<TimeCondition> CODEC = Codec.either(Keyword.CODEC, RANGE_ARRAY_CODEC).flatXmap(
            either -> either.map(
                    ...
                    list -> createRange(list.getFirst(), list.get(1))
            ),
    ...
    public TimeCondition {
        ...
        Optional<Keyword> resolvedKeyword = keyword;
        if (resolvedKeyword.isPresent()) {
            Keyword value = resolvedKeyword.get();
            if (value.startTick != startTick || value.endTick != endTick) {
                throw new IllegalArgumentException(ERROR_KEYWORD_RANGE_MISMATCH);
            }
        } else {
            resolvedKeyword = Keyword.fromRange(startTick, endTick);
        }
